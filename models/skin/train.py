...
from models.skin.config import DATA_DIR, RESULTS_DIR, MODEL_SAVE_PATH, BATCH_SIZE, EPOCHS, LEARNING_RATE, SEED
from models.skin.dataset import SkinDataset
from models.skin.model import SkinClassifier
from models.skin.evaluate import evaluate_model

def train():
    torch.manual_seed(SEED)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Training Skin Model on device: {device}")
    
    train_dataset = SkinDataset(DATA_DIR, split="train")
    val_dataset = SkinDataset(DATA_DIR, split="val")
    
    if len(train_dataset) == 0:
        print("Error: Skin train dataset is empty. Run python data/prepare_datasets.py first.")
        return
    ...
    print(f"Saved best Skin Model to {MODEL_SAVE_PATH}")
